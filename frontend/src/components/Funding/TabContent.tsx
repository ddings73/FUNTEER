...
import { targetMoneyLevelType } from '../../types/funding';

type TabContentPropsType = {
  data: targetMoneyLevelType;
  ...
  removeTodo: (index: number, level: string) => void;
  todoText: string;
  level: string;
};
function TabContent(props: TabContentPropsType) {
  const { data, onChangeTextHandler, onChangeTodoHandler, onKeyDownHandler, addTodos, level, todoText, removeTodo } = props;

  const onClickAddTodos = () => {
    console.log(todoText);

    if (todoText.length > 0) addTodos(level);
  };

  const onKeydownAddTodos = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && todoText.length > 0) onKeyDownHandler(e, level);
  };

  const onClickRemoveTodo = (index: number, level: string) => {
    removeTodo(index, level);
  };
  return (
        ...
        <div className={styles['todo-list-box']}>
          ...
          {data.descriptions.map((todo, index) => (
            ...
            <p key={index} className={styles['todo-contents']}>
              ...
              <IconButton size="medium" sx={{ color: 'red' }} onClick={() => onClickRemoveTodo(index, level)}>
                {' '}

	...
	public SseEmitter subscribe( String lastEventId) {
		...
		String emitterId = makeTimeIncludeId(email);
			...
			if (alarmRepository.findAllEmitterStartWithByEmail(email) != null){
				...
				emitter = alarmRepository.save(emitterId, new SseEmitter(Long.MAX_VALUE)); //id가 key, SseEmitter가 value
			}
			else {
				emitter = alarmRepository.save(emitterId, new SseEmitter(Long.MAX_VALUE)); //id가 key, SseEmitter가 value
			}
		...
		private void sendLostData(String lastEventId, String email, String emitterId, SseEmitter emitter) {
			...
			Map<String, Object> eventCaches = alarmRepository.findAllEventCacheStartWithByEmail(String.valueOf(email));
